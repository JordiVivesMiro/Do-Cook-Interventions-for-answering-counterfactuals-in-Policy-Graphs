...
class Node:
    def __init__(self, node_id: int, prob: float, state_rep, nodes: List,
                 transitions: Dict, pg):
    ...
    def get_action_probability(self):
        try:
            ...
            print(f'Warning: State {self.node_id} has no sampled successors which were asked for')
            return dict()
    ...
    def sample_next_state(self):
        try:
            ...
            result = {(action, next_id): p for action, possible_nexts in transitions.items()
                      for next_id, p in possible_nexts.items()}
            result_idx = np.random.choice(len(result.keys()), 1, p=list(result.values()))
            s_action, s_next = list(result.keys())[result_idx[0]]
            return s_action, s_next
        except KeyError:
            raise ValueError(f'State {self.node_id} has no known successors in the edge file')
    ...
    def descendants(self, action=None):  # List[Node, str(action)]
        if action is not None:
            return [(self.nodes[descendant_idx], action)
    ...
    def answer_why(self, action_idx: str, c_threshold, probability_threshold=0):
        ...
        if len(attr_ints) == 0:
            ...
            successors = [(s, p, self.nodes[s].intention) for s, p in
            ...
            for _,p,_ in successors:
                p_a_s +=p
            successors = [(s, p/p_a_s, i) for s, p, i in successors]# successors contain P(s'|a,s)
            int_increase = {}
            for d, val in attr_ints.items():
                ...
                for _, p, ints in successors:
                    ...
                    int_increase[d]['expected_pos_increase'] += p * ints[d] if ints[d] >= val else 0
    ...
    def _answer_how_stochastic(self, desires, c_threshold):
        ...
        for desire in desires:
            ...
            clause, action_idx = desire.clause, desire.action_idx
            if self.check_desire(desire_clause=clause, action_id=action_idx) is not None:
                how_paths_per_desire[d_name] = [[action_idx, None, None], 'SUCCESS']  # Desire will get fulfilled here
                continue
            ...
            how_paths_per_desire[d_name] = [[action, next_state, next_state.intention[d_name]]] + tail
        return how_paths_per_desire


class PropoNode(Node):
    def __init__(self, node_id: int, prob: float, state_rep,
                 nodes: List, transitions: Dict, pg):  # TODO: state_rep set of enum
